...
function playPcm(bytes, ctx) {
  ...
  const when = Math.max(ctx.currentTime + 0.01, nextPlayTime); source.start(when); nextPlayTime = when + buffer.duration;
}
async function stopAudio() {
  if (starting) return;
  try { const target = await selectedChild(); if (target) await updateDoc(refs(target).request, { status: 'stop_requested', updatedAt: Date.now() }); } catch (_) {}
  setStatus('⏳ To‘xtatilmoqda…'); stopBtn.disabled = true; await cleanupAudio(true);
}
async function cleanupAudio(closeContext) {
  requestUnsub?.(); requestUnsub = null; audioUnsub?.(); audioUnsub = null; currentSession = null; seen.clear(); nextPlayTime = 0;
  if (closeContext && audioContext) { await audioContext.close().catch(() => {}); audioContext = null; }
  startBtn.disabled = false; stopBtn.disabled = true;
}
function buildUi() {
  const dashboard = document.getElementById('dashboard'); const tabs = document.getElementById('tabs'); if (!dashboard || !tabs) return;
  const screenshotTab = tabs.querySelector('[data-tab="ss"]'); if (!screenshotTab) return;
  if (!voiceTab?.isConnected) {
    voiceTab = document.createElement('button'); voiceTab.type = 'button'; voiceTab.dataset.tab = 'voice';
    voiceTab.innerHTML = '<span aria-hidden="true" style="font-size:24px;line-height:24px">🎙️</span><br>Ovoz'; voiceTab.title = 'Jonli eshitish';
    voiceTab.onclick = () => { modal?.classList.add('active'); setStatus('Tayyor'); }; tabs.insertBefore(voiceTab, screenshotTab.nextSibling);
  } else if (voiceTab.previousElementSibling !== screenshotTab) tabs.insertBefore(voiceTab, screenshotTab.nextSibling);
  if (!document.getElementById('voiceTabStyle')) {
    const style = document.createElement('style'); style.id = 'voiceTabStyle';
    style.textContent = '#tabs{grid-template-columns:repeat(5,1fr)}#tabs [data-tab="voice"]{border-color:#dce2eb;background:#fff;color:#596578}#tabs [data-tab="voice"]:active{transform:scale(.97)}@media(max-width:430px){#tabs{grid-template-columns:repeat(3,1fr)}}'; document.head.appendChild(style);
  }
  if (card?.isConnected) card.remove(); card = null;
  if (!modal?.isConnected) {
    ...
    document.body.appendChild(modal); statusEl = modal.querySelector('#micStatus'); startBtn = modal.querySelector('#micStart'); stopBtn = modal.querySelector('#micStop');
    startBtn.onclick = openAudio; stopBtn.onclick = stopAudio; modal.querySelector('#micClose').onclick = () => { if (!stopBtn.disabled) stopAudio(); modal.classList.remove('active'); };
  }
}
const observer = new MutationObserver(buildUi); observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class'] }); setInterval(buildUi, 800); setTimeout(buildUi, 600);
