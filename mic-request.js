...
function refs(target) {
  ...
  return {
    request: doc(db, ...base, 'mic_requests', 'current'),
    audio: collection(db, ...base, 'mic_audio')
  };
}
...
async function openAudio() {
  ...
  try {
    ...
    requestUnsub = onSnapshot(r.request, snap => {
      ...
      if (d.status === 'stopped') { setStatus('To‘xtatildi'); cleanupAudio(false); }
      if (d.status === 'failed') { setStatus('❌ Mikrofon ulanmagan'); cleanupAudio(false); }
...
async function cleanupAudio(closeContext) {
  ...
  seen.clear();
  nextPlayTime = 0;
  if (closeContext && audioContext) await audioContext.close().catch(() => {});
  if (closeContext) audioContext = null;
  startBtn.disabled = false;
  stopBtn.disabled = true;
}
