...
class SettingsActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Screenshot nazorati — mavjud Settings XMLni buzmasdan alohida qatorni runtime qo'shamiz.
        val content = findViewById<android.view.ViewGroup>(android.R.id.content)
        val root = content.getChildAt(0) as? android.view.ViewGroup
    ...
    private fun showThemePickerDialog() {
        ...
        AlertDialog.Builder(this).setTitle("Mavzu").setSingleChoiceItems(options, if (current) 1 else 0) { dialog, which ->
            val isLight = which == 1
            getSharedPreferences("oila_nazorati", Context.MODE_PRIVATE).edit().putBoolean("light_theme", isLight).apply()
            AppCompatDelegate.setDefaultNightMode(if (isLight) AppCompatDelegate.MODE_NIGHT_NO else AppCompatDelegate.MODE_NIGHT_YES); dialog.dismiss()
        }.setNegativeButton("Bekor qilish", null).show()
    }
    private fun showEnterCodeDialog() {
        val input = EditText(this).apply { hint = "Oila kodi"; setText(FirebaseRepo.familyCode.orEmpty()) }
        val container = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; val pad=(16*resources.displayMetrics.density).toInt(); setPadding(pad,pad,pad,0); addView(input) }
        AlertDialog.Builder(this).setTitle("Oila kodi").setView(container).setPositiveButton("Yuklash") { _, _ -> input.text?.toString()?.trim()?.uppercase()?.takeIf { it.isNotBlank() }?.let { saveFamilyCodeAndReturn(it) } }.setNegativeButton("Bekor qilish", null).show()
    }
    private fun createNewFamilyCode() {
        AlertDialog.Builder(this).setTitle("Yangi oila kodi yaratish").setMessage("Bu yangi, hozirgi kodga bog'liq bo'lmagan oila yaratadi. Shu oilaga yana farzand qo'shish uchun yangi kod yaratmang.").setPositiveButton("Baribir yarataman") { _, _ -> generateAndCreateFamily() }.setNegativeButton("Bekor qilish", null).show()
    }
    ...
    private fun saveFamilyCodeAndReturn(code:String) {
        FirebaseRepo.familyCode=code; FirebaseRepo.childId=null
        getSharedPreferences("oila_nazorati",Context.MODE_PRIVATE).edit().putString("family_code",code).remove("child_id").apply()
        startActivity(Intent(this,ParentDashboardActivity::class.java)); finish()
    }
